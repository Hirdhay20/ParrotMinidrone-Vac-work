...
#define flightControlSystem_CALL_EVENT_m (-1)
...
{
  rtmSampleHitPtr[1] = rtmStepTask(flightControlSystem_M, 1);
   ...
   P_ControlSystem_flightControlSystem_T *localP)
{
  ...
  /* InitializeConditions for UnitDelay: '<S242>/Output' */
  ...
  localDW->sfEvent_h = flightControlSystem_CALL_EVENT_a;
  localB->Xo = 0.0;
  localB->Yo = 0.0;
  localB->Zo = 0.0;
  localDW->i = 0.0;
  ...
  ZCE_ControlSystem_flightControlSystem_T *localZCE)
{
  ...
  localB->nicemeasurementornewupdateneeded = ((fabs(localDW->Delay2_DSTATE -
    localB->u_h) <= localP->outlierJump_const) && (localB->invertzaxisGain <
    -flightControlSystem_P.Sensors.altSensorMin) && ((!(fabs
    (localB->pressureFilter_IIR - localDW->Delay2_DSTATE) >=
  ...
  if ((localB->Abs < 0.0F) && (localP->Constant_Value_h > localB->rtb_Switch_p))
  {
    localB->Abs_c = -flightControlSystem_rt_powf_snf(-localB->Abs,
      localP->Constant_Value_h, localB);
  } else {
    localB->Abs_c = flightControlSystem_rt_powf_snf(localB->Abs,
      localP->Constant_Value_h, localB);
   ...
   */
  localB->pitchrollerror = (real32_T)sqrt((localB->rtb_Switch_c + localB->Abs_c)
  ...
  if (localB->Compare_jk) {
    localB->Abs_c = rtu_Sensors->VisionSensors.opticalFlow_data[0];
  } else {
    localB->Abs_c = localP->opticalFlowErrorCorrect_Gain * localB->rtb_Switch_c;
  }

  localB->Switch_k[0] = localB->Abs_c;
  ...
  for (localB->j = 0; localB->j <= 0; localB->j += 2) {
    _mm_storeu_pd(&localB->rtb_VectorConcatenate_c[localB->j], _mm_add_pd
      ...
      _mm_set1_pd(localB->unnamed_idx_1)), _mm_mul_pd(_mm_set_pd
   ...
   *  Switch: '<S442>/Switch'
   */
  localB->Compare_jkn = ((localB->Abs_c !=
  ...
  for (localB->j = 0; localB->j <= 0; localB->j += 2) {
    _mm_storeu_pd(&localB->rtb_VectorConcatenate_c[localB->j], _mm_sub_pd
      ...
      Reshape9to3x3columnmajor[localB->j]), _mm_set1_pd(localB->unnamed_idx_1))),
  ...
  for (localB->j = 2; localB->j < 3; localB->j++) {
    localB->rtb_VectorConcatenate_c[localB->j] = localB->FIR_IMUaccel[localB->j]
      - ((localB->Reshape9to3x3columnmajor[localB->j + 3] * localB->u_h +
          localB->Reshape9to3x3columnmajor[localB->j] * localB->unnamed_idx_1) +
    ...
    for (localB->j = 0; localB->j < 2; localB->j++) {
      localB->Conversion_g[localB->j] = localB->
        ...
        * localB->rtb_Switch_c) + localP->C_Value_i2[localB->j + 4] *
        ...
        localB->Abs_c + localP->D_Value_h[localB->j] * localB->rtb_Switch_p));
    }
  ...
  for (localB->j = 0; localB->j <= 0; localB->j += 4) {
    ...
    _mm_storeu_ps(&localB->Reshape_k[localB->j], _mm_add_ps(_mm_add_ps
      ...
      (&localP->B_Value_f[localB->j + 4]), _mm_set1_ps(localB->Abs_c)),
   ...
   *  Gain: '<S167>/Proportional Gain'
   */
  localB->Abs_c = localB->SaturationThrust1 - localB->Reshapexhat_d[0];
  localB->Conversion_g[0] = localB->Abs_c;

  /* Sum: '<S225>/Sum' incorporates:
   *  DiscreteIntegrator: '<S216>/Integrator'
   *  Gain: '<S221>/Proportional Gain'
   */
  localB->rtb_Switch_p = localP->Velocitycontroller_P * localB->Abs_c +
    localDW->Integrator_DSTATE[0];
   ...
   *  Gain: '<S167>/Proportional Gain'
   */
  localB->Abs_c = localB->SaturationThrust1 - localB->Reshapexhat_d[1];
   ...
   *  Sum: '<S9>/Add'
   */
  localB->unnamed_idx_1_b = (localP->P_yaw_Gain * localB->rtb_sincos_o1_idx_2 +
  ...
  localB->unnamed_idx_1_b = localB->Reshapexhat_d[1];
  ...
  for (localB->j = 0; localB->j < 3; localB->j++) {
    ...
    localB->Product_k[localB->j] = (localB->VectorConcatenate[3 * localB->j + 1]
      * localB->unnamed_idx_1_b + localB->VectorConcatenate[3 * localB->j] *
  ...
  if (localB->nicemeasurementornewupdateneeded) {
    ...
    _mm_storeu_pd(&localB->Product3[0], _mm_mul_pd(_mm_loadu_pd
      ...
      (localP->D_Value * localB->Sum[2] + (localP->C_Value[0] *
   ...
   *  Sum: '<S10>/Sum19'
   */
  localB->rtb_sincos_o1_idx_0 = localP->Velocitycontroller_I * localB->Abs_c;
   ...
   *  UnitDelay: '<S14>/Output'
   */
  localB->FixPtSwitch_jj = (uint16_T)(localDW->Output_DSTATE_a +
    localP->FixPtConstant_Value_g);

  /* Switch: '<S16>/FixPt Switch' */
  if (localB->FixPtSwitch_jj > localP->WrapToZero_Threshold_c) {
    /* Sum: '<S15>/FixPt Sum1' incorporates:
     *  Constant: '<S16>/Constant'
     *  Switch: '<S16>/FixPt Switch'
     */
    localB->FixPtSwitch_jj = localP->Constant_Value_k;
  }
   ...
   *  Sum: '<S4>/Subtract'
   */
  localB->Abs_c = (real32_T)fabs(localB->Abs - localB->Reshapexhat_d[0]);
  ...
  localB->Compare_jk = ((localB->Abs > localP->CompareToConstant4_const) &&
                        (localB->Abs_c > localP->CompareToConstant2_const));
   ...
   *  Sum: '<S4>/Subtract1'
   */
  localB->Abs_c = (real32_T)fabs(localB->Abs - localB->Reshapexhat_d[1]);
   ...
   */
  localB->Compare_jkn = ((localB->Abs_c > localP->CompareToConstant3_const) &&
   ...
   *  Product: '<S4>/  '
   */
  localB->Add = localB->u_h * localDW->DelayOneStep_DSTATE + localB->u_h;
  ...
  } else if (localB->Add > localP->u0continuousOFerrors_const) {
   ...
   *  UnitDelay: '<S242>/Output'
   */
  localB->FixPtSwitch = localDW->Output_DSTATE_g + localP->FixPtConstant_Value;

  /* Switch: '<S249>/FixPt Switch' */
  if (localB->FixPtSwitch > localP->WrapToZero_Threshold) {
    /* Sum: '<S248>/FixPt Sum1' incorporates:
     *  Constant: '<S249>/Constant'
     *  Switch: '<S249>/FixPt Switch'
     */
    localB->FixPtSwitch = localP->Constant_Value_a;
  }
   ...
   *  UnitDelay: '<S514>/Output'
   */
  localB->FixPtSwitch_j = localDW->Output_DSTATE_n +
    localP->FixPtConstant_Value_p;

  /* Switch: '<S516>/FixPt Switch' */
  if (localB->FixPtSwitch_j > localP->WrapToZero_Threshold_k) {
    /* Sum: '<S515>/FixPt Sum1' incorporates:
     *  Constant: '<S516>/Constant'
     *  Switch: '<S516>/FixPt Switch'
     */
    localB->FixPtSwitch_j = localP->Constant_Value_e;
  }
   ...
   *  UnitDelay: '<S432>/Output'
   */
  localB->FixPtSwitch_m = localDW->Output_DSTATE + localP->FixPtConstant_Value_m;

  /* Switch: '<S435>/FixPt Switch' */
  if (localB->FixPtSwitch_m > localP->WrapToZero_Threshold_e) {
    /* Sum: '<S434>/FixPt Sum1' incorporates:
     *  Constant: '<S435>/Constant'
     *  Switch: '<S435>/FixPt Switch'
     */
    localB->FixPtSwitch_m = localP->Constant_Value_jy;
  }
   ...
   P_ControlSystem_flightControlSystem_T *localP)
{
  ...
  /* Update for UnitDelay: '<S432>/Output' */
  localDW->Output_DSTATE = localB->FixPtSwitch_m;
  ...
  /* Update for UnitDelay: '<S514>/Output' */
  localDW->Output_DSTATE_n = localB->FixPtSwitch_j;
  for (k = 0; k < 2; k++) {
  ...
  localDW->Output_DSTATE_a = localB->FixPtSwitch_jj;
  ...
  /* Update for UnitDelay: '<S242>/Output' */
  localDW->Output_DSTATE_g = localB->FixPtSwitch;

  /* Update for Delay: '<S4>/Delay One Step' */
  localDW->DelayOneStep_DSTATE = localB->Add;
}
...
void flightControlSystem_update0(void) /* Sample time: [0.005s, 0.0s] */
{
  ...
  flightControlSystem_M->Timing.t[0] = flightControlSystem_M->Timing.clockTick0 *
...
void flightControlSystem_output1(void) /* Sample time: [0.2s, 0.0s] */
{
   ...
   */
  flightControlSystem_DW.sfEvent = flightControlSystem_CALL_EVENT_m;
  for (i = 0; i < 19200; i++) {
  ...
  flightControlSystem_DW.sfEvent_j = flightControlSystem_CALL_EVENT_m;
  for (i = 0; i < 57600; i++) {
    if (isredDetected != 0) {
...
void flightControlSystem_update1(void) /* Sample time: [0.2s, 0.0s] */
{
  ...
  flightControlSystem_M->Timing.t[1] = flightControlSystem_M->Timing.clockTick1 *
    flightControlSystem_M->Timing.stepSize1 +
    flightControlSystem_M->Timing.clockTickH1 *
    flightControlSystem_M->Timing.stepSize1 * 4294967296.0;
}
...
void flightControlSystem_initialize(void)
{
  ...
  flightControlSystem_DW.captured_not_empty = true;
  flightControlSystem_DW.doneDoubleBufferReInit_j = false;
  flightControlSystem_DW.sfEvent_j = flightControlSystem_CALL_EVENT_m;
...
{
  flightControlSystem_output(tid);
}

void MdlUpdate(int_T tid)
{
  flightControlSystem_update(tid);
}
...
RT_MODEL_flightControlSystem_T *flightControlSystem(void)
{
  /* Registration code */
  ...
  {
    int_T *mdlTsMap = flightControlSystem_M->Timing.sampleTimeTaskIDArray;
    mdlTsMap[0] = 0;
    mdlTsMap[1] = 1;
    flightControlSystem_M->Timing.sampleTimeTaskIDPtr = (&mdlTsMap[0]);
    flightControlSystem_M->Timing.sampleTimes =
      (&flightControlSystem_M->Timing.sampleTimesArray[0]);
    flightControlSystem_M->Timing.offsetTimes =
      (&flightControlSystem_M->Timing.offsetTimesArray[0]);

    /* task periods */
    flightControlSystem_M->Timing.sampleTimes[0] = (0.005);
    flightControlSystem_M->Timing.sampleTimes[1] = (0.2);

    /* task offsets */
    flightControlSystem_M->Timing.offsetTimes[0] = (0.0);
    flightControlSystem_M->Timing.offsetTimes[1] = (0.0);
  }

  rtmSetTPtr(flightControlSystem_M, &flightControlSystem_M->Timing.tArray[0]);

  {
    int_T *mdlSampleHits = flightControlSystem_M->Timing.sampleHitArray;
    int_T *mdlPerTaskSampleHits =
      flightControlSystem_M->Timing.perTaskSampleHitsArray;
    flightControlSystem_M->Timing.perTaskSampleHits = (&mdlPerTaskSampleHits[0]);
    mdlSampleHits[0] = 1;
    flightControlSystem_M->Timing.sampleHits = (&mdlSampleHits[0]);
  }

  rtmSetTFinal(flightControlSystem_M, 100.0);
  flightControlSystem_M->Timing.stepSize0 = 0.005;
  flightControlSystem_M->Timing.stepSize1 = 0.2;

  /* External mode info */
  flightControlSystem_M->Sizes.checksums[0] = (1343623589U);
  flightControlSystem_M->Sizes.checksums[1] = (3692005055U);
  flightControlSystem_M->Sizes.checksums[2] = (1641993464U);
  flightControlSystem_M->Sizes.checksums[3] = (3826722563U);
  ...
  flightControlSystem_M->solverInfoPtr = (&flightControlSystem_M->solverInfo);
  flightControlSystem_M->Timing.stepSize = (0.005);
  rtsiSetFixedStepSize(&flightControlSystem_M->solverInfo, 0.005);
  rtsiSetSolverMode(&flightControlSystem_M->solverInfo, SOLVER_MODE_MULTITASKING);

  /* block I/O */
  flightControlSystem_M->blockIO = ((void *) &flightControlSystem_B);
  ...
  /* parameters */
  flightControlSystem_M->defaultParam = ((real_T *)&flightControlSystem_P);

  /* states (dwork) */
  flightControlSystem_M->dwork = ((void *) &flightControlSystem_DW);
  ...
  /* Initialize Sizes */
  flightControlSystem_M->Sizes.numContStates = (0);/* Number of continuous states */
  flightControlSystem_M->Sizes.numY = (5);/* Number of model outputs */
  flightControlSystem_M->Sizes.numU = (38402);/* Number of model inputs */
  flightControlSystem_M->Sizes.sysDirFeedThru = (1);/* The model is direct feedthrough */
  flightControlSystem_M->Sizes.numSampTimes = (2);/* Number of sample times */
  flightControlSystem_M->Sizes.numBlocks = (638);/* Number of blocks */
  flightControlSystem_M->Sizes.numBlockIO = (74);/* Number of block outputs */
  flightControlSystem_M->Sizes.numBlockPrms = (540);/* Sum of parameter "widths" */
  return flightControlSystem_M;
}
